...
case $response in
    [yY][eE][sS]|[yY]) # choose WordPress language
echo -e "${GREEN}Please, choose WordPress language you need (set RUS or ENG): "
  ...
  chmod -R 777 /var/www/$username/$websitename/www/wp-content/uploads
    ...
    *) echo -e "${RED}WordPress were not downloaded & installed. You can do this manually or re run this script.${NC}" ;;
esac
}
...
echo -e "${YELLOW}On this step we going to add basic set of plugins...${NC}"

read -r -p "Do you want to install basic plugins? [y/N] " response
case $response in
    [yY][eE][sS]|[yY]) ##################
  ...
  SITEMAP="`curl wordpress.org/plugins/google-sitemap-generator/ | grep downloads.wordpress.org/plugin/google-sitemap-generator.*.*.*.zip | awk '{print $3}' | sed -ne 's/.*\(http[^"]*.zip\).*/\1/p'`"
  wget $SITEMAP -O /tmp/sitemap.zip && unzip /tmp/sitemap.zip -d /tmp/sitemap
  mv /tmp/sitemap/* /var/www/$username/$websitename/www/wp-content/plugins/

  AUTOPOSTER="wget downloads.wordpress.org/plugin/social-networks-auto-poster-facebook-twitter-g.zip"
  wget $AUTOPOSTER -O /tmp/snap.zip
  unzip /tmp/snap.zip -d /tmp/snap
  mv /tmp/snap/* /var/www/$username/$websitename/www/wp-content/plugins/

  ADDTOANY="`curl wordpress.org/plugins/add-to-any/ | grep downloads.wordpress.org/plugin/add-to-any.*.*.zip | awk '{print $3}' | sed -ne 's/.*\(http[^"]*.zip\).*/\1/p'`"
  ...
  WATERMARK="`curl wordpress.org/plugins/easy-watermark/ | grep downloads.wordpress.org/plugin/easy-watermark.*.*.*.zip | awk '{print $3}' | sed -ne 's/.*\(http[^"]*.zip\).*/\1/p'`"
