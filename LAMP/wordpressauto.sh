...
#Welcome message
clear
echo -e "WordPress & LAMP stack installation and configuration"
...
echo -e "${YELLOW}Adding separate user & creating website home folder for secure running of your website...${NC}"
  ...
  groupadd $username
  adduser --home /var/www/ --ingroup $username $username
...
#read -r -p "Do you want to install WordPress & automatically set optimal and secure configuration with basic set of plugins? [y/N] " response; case $response in; [yY][eE][sS]|[yY]) 
...
#;; *) echo -e "${RED}WordPress and plugins were not downloaded & installed. You can do this manually or re run this script.${NC}" ;; esac
