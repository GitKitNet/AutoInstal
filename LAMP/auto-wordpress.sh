...
function OSInfo() {
	...
	if [ "$1" -eq "OS" ]; then echo "${OS}";
	elif [ "$1" -eq "REL" ] && [ "$2" -eq "REL" ]; then echo "${REL}";
	elif [ -z "$1" ]; then echo "${OS} ${REL}";
	fi;
};
# OSInfo OS REL
...
function TIMER() {
	if [[ "$1" =~ ^[[:digit:]]+$ ]]; then T="$1"; else T="10"; fi;
	SE="$((1 * ${T}))" && SC='\033[0K\r';
...
#Welcome message
clear
echo -e "WordPress & LAMP installation and configuration"
sleep 4

clear
echo -e "${YELLOW}Checking ALL packages...${NC}"
sleep 4
...
FAIL2BAN=$(dpkg-query -W -f='${Status}' fail2ban 2>/dev/null | grep -c "ok installed")
  if [ "$FAIL2BAN" -eq 0 ]; then echo -e "${YELLOW}Installing fail2ban${NC}" && apt-get install fail2ban --yes;
elif [ "$FAIL2BAN" -eq 1 ]; then echo -e "${GREEN}fail2ban is installed!${NC}"
fi

APACHE2=$(dpkg-query -W -f='${Status}' apache2 2>/dev/null | grep -c "ok installed")
  if [ "$APACHE2" -eq 0 ]; then echo -e "${YELLOW}Installing apache2${NC}" && apt-get install apache2 php5 --yes;
elif [ "$APACHE2" -eq 1 ]; then echo -e "${GREEN}apache2 is installed!${NC}"
fi

MYSQL=$(dpkg-query -W -f='${Status}' mysql-server 2>/dev/null | grep -c "ok installed")
  if [ "$MYSQL" -eq 0 ]; then echo -e "${YELLOW}Installing mysql-server${NC}" && apt-get install mysql-server --yes;
elif [ "$MYSQL" -eq 1 ]; then echo -e "${GREEN}mysql-server is installed!${NC}"
fi

PHP5CURL=$(dpkg-query -W -f='${Status}' php5-curl 2>/dev/null | grep -c "ok installed")
  if [ "$PHP5CURL" -eq 0 ]; then echo -e "${YELLOW}Installing php5-curl${NC}" && apt-get install php5-curl --yes;
...
WGET=$(dpkg-query -W -f='${Status}' wget 2>/dev/null | grep -c "ok installed")
  if [ "$WGET" -eq 0 ]; then echo -e "${YELLOW}Installing wget${NC}" && apt-get install wget --yes;
elif [ "$WGET" -eq 1 ]; then echo -e "${GREEN}wget is installed!${NC}"
fi

CURL=$(dpkg-query -W -f='${Status}' curl 2>/dev/null | grep -c "ok installed")
  if [ "$CURL" -eq 0 ]; then echo -e "${YELLOW}Installing curl${NC}" && apt-get install curl --yes;
...
# -------------------------------------
HTACCESS="/var/www/$websitename/.htaccess"
if [ -f "$HTACCESS" ]; then
...
# -------------------------------------
ROBOTSTXT="/var/www/$websitename/robots.txt"
if [ -f "$ROBOTSTXT" ]; then
...
EOL
	...
	echo -e "${GREEN}File robots.txt was succesfully created!${NC}";
	echo -e "${GREEN}Setting correct rights on user's home directory and 755 rights on robots.txt${NC}";
	sleep 3
fi
...
if [ -f "$JAILCONF" ]; then
	echo -e "${GREEN}Create fail2ban...${NC}"
	sleep 3
	touch ${JAILCONF}
else
	echo -e "${GREEN}Configuring fail2ban...${NC}"
	sleep 3
	cp ${JAILCONF} ${JAILCONF}-old
fi;
...
# Configuring apache2 prefork & worker modules
...
# ----------------------------------------
echo -e "${GREEN}Adding user & database for WordPress, setting wp-config.php...${NC}" && sleep 3
echo -e "Set username for database: "
db_user=db_$(echo ${websitename} | tr "." "_" | tr "-" "_")
# read db_user
