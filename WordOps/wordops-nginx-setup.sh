...
[ "$(id -u)" != "0" ] && {
	echo -e "ERROR: You must be root user to install the software."
	echo -en "\n\nUse 'sudo su - root' to login as root"
	exit 1
}
...
if [ "$INTERACTIVE_SETUP" = "y" ]; then
	if [ -z "$(command -v mysqladmin)" ]; then
		...
		if [ "$MARIADB_SERVER_INSTALL" = "n" ]; then
			echo -en "\n Do you want to install MariaDB-client for a remote database ? (y/n)\n"
...
if [ "$MARIADB_SERVER_INSTALL" = "y" ]; then
	[ -z "$MARIADB_VERSION_INSTALL" ] && { MARIADB_VERSION_INSTALL="10.3"; }
